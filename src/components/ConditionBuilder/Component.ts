...
import ConditionBuilderWidget, { Conflict } from './ConditionBuilderWidget';

export default function createConditionBuilder() {
    const BaseComponent = (Components as any).components.base;

    class ConditionBuilderComponent extends (BaseComponent as any) {
        static schema() {
            return {
        ...
        _valueKey = '';
        _conflict: Conflict = null;
        _watchingConflicts = false;
        ...
        _getConflict(): Conflict {
            ...
            if (data.customConditional) return 'custom';
            const conditional = data.conditional || {};
            if (conditional.when) return 'simple';
            const conditions = conditional.conditions || [];
            if (Array.isArray(conditions) && conditions.some((c: any) => c?.component && c?.operator)) {
                return 'simple';
            }
            return null;
        }
        ...
        _watchConflicts() {
            if (this._watchingConflicts) return;
        ...
        _getFields(): Field[] {
            try {
                ...
                const addressable = (key: any) =>
                    typeof key === 'string' && key !== '' && !key.includes('.');

                const fields: Field[] = [];
                const editedKey = this.root?.data?.key as string | undefined;

                eachComponent(editForm.components, (component: any) => {
                    if (!component.key) return;
                    if (component.key === editedKey || component.key === this.component.key) return;
                    if (!addressable(component.key)) return;
                    if (component.input === false) return;
                    if (SKIP_TYPES.has(component.type)) return;
                    ...
                    if (component.type === 'simplesurvey') {
                        const surveyValues = (component.values || []).map((v: any) => ({
                            name: v.value, label: v.label, value: v.value,
                        }));
                        (component.questions || []).forEach((q: any) => {
                            if (!addressable(q.value)) return;
                            fields.push({
                        ...
                        const statements: Array<{id: string; label: string}> = component.statements || [];
                        ...
                        ['simpleradios', 'simpleselect'].includes(component.type) &&
                        ...
                        fields.push({
                            ...
                            valueEditorType: 'select',
                            values: component.values.map((v: any) => ({
                                name: v.value, label: v.label, value: v.value,
                            })),
                        });
